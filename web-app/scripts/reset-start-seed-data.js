...
process.env.DISABLE_CREATE_INDEXES = '1';
...
const STUDENT_REG = (process.env.SEED_STUDENT_REG || '').trim();
...
async function main() {
  ...
  const studentRes = await students.deleteMany({
    $or: [
      { email: STUDENT_EMAIL },
      ...(STUDENT_REG ? [{ registrationNumber: STUDENT_REG }] : [])
    ]
  });
  ...
  console.log(JSON.stringify({
    ...
    universityName: UNIVERSITY_NAME,
    studentEmail: STUDENT_EMAIL,
    studentRegistrationNumber: STUDENT_REG || null
  }, null, 2));

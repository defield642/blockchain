...
async function issueCertificate(certData) {
    ...
    certData.studentRegistrationNumber = (certData.studentRegistrationNumber || "").trim();
    certData.studentNationalIdNumber = (certData.studentNationalIdNumber || "").trim();
    ...
    if (!studentObj) {
        try {
            ...
            const tempPassword = certData.studentNationalIdNumber || crypto.randomBytes(12).toString('hex');
            studentObj = await students.create({
                name: certData.studentName,
                email: certData.studentEmail,
                password: tempPassword,
                registrationNumber: certData.studentRegistrationNumber || undefined,
                usesNationalIdPassword: Boolean(certData.studentNationalIdNumber),
        ...
        let needsSave = false;
        if (certData.studentRegistrationNumber && !studentObj.registrationNumber) {
            studentObj.registrationNumber = certData.studentRegistrationNumber;
            needsSave = true;
        }
        if (certData.studentNationalIdNumber) {
            const isLegacyAutoProvisioned = typeof studentObj.isAutoProvisioned === "undefined";
            if (studentObj.isAutoProvisioned || isLegacyAutoProvisioned) {
                studentObj.password = certData.studentNationalIdNumber;
                studentObj.isAutoProvisioned = false;
                studentObj.usesNationalIdPassword = true;
                needsSave = true;
            }
        }
        if (needsSave) {
    ...
    if (certData.studentNationalIdNumber) {
        try {
            await mailService.sendStudentIssueAlert(certData.studentEmail, certData.studentNationalIdNumber);
        } catch (e) {
            logger.error(`Failed to send student issue alert to ${certData.studentEmail}: ${e && e.message ? e.message : e}`);
        }
    }

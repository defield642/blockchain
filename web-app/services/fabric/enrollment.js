...
async function registerUser(email){
    try {
        ...
        if (userIdentity) {
            const existingKeys = walletUtils.loadHexKeysFromWallet(email);
            if (existingKeys) {
                logger.info(`User ${email} already exists in wallet. Reusing keys.`);
                return existingKeys;
            }
            throw Error(`An identity for the user ${email} already exists in the wallet, but no key file was found. Delete the wallet entry and retry.`);
        }
